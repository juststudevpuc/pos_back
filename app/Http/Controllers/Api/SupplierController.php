<?php

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Models\Supplier;
use App\Traits\ApiResponseTrait;
use Illuminate\Http\Request;

class SupplierController extends Controller
{
    use ApiResponseTrait;

    public function search(Request $req)
    {
        $query = $req->query("q");
        $supplier = Supplier::where("name", "like", "%" . $query . "%")->get();
        return response()->json([
            "Query" => $query,
            "data" => $supplier,
            "message" => "Get supplier successfully."
        ]);
    }
    /**
     * Display a listing of the resource.
     */
    public function index()
    {
        $supplier = Supplier::query()->get();

        return $this->successApiResponse($supplier, "Get supplier successfully", 200);
    }

    public function storeBulk(Request $req)
    {
        $validate = $req->validate([
            "suppliers" => "required|array",
            "suppliers.*.name" => "required|string",
            "suppliers.*.email" => "required|string",
            "suppliers.*.address" => "required|string",
            "suppliers.*.tel" => "required|string",
            "suppliers.*.website" => "required|string",
        ]);

        foreach ($validate["suppliers"] as $item) {
            $supplier = Supplier::create($item);
        }

        return response()->json([
            "data" => $supplier,
            "message" => "Created supplier successfully."
        ], 201);
    }

    /**
     * Store a newly created resource in storage.
     */
    public function store(Request $request)
    {
        $validate = $request->validate([
            "name" => "required|string|min:3|max:255",
            "email" => "required|string|unique:suppliers,email",
            "address" => "required|string|min:2|max:255",
            "tel" => "required|string|min:3|max:255",
            "website" => "required|string|min:3|max:255",
        ]);

        $supplier = Supplier::create($validate);

        return response()->json([
            "data" => $supplier,
            "message" => "Created supplier successfully."
        ], 201);
    }

    /**
     * Display the specified resource.
     */
    public function show(string $id)
    {
        $supplier = Supplier::find($id);

        if (!$supplier) {
            return response()->json([
                "message" => "Cetegory is not found."
            ], 404);
        }

        return response()->json([
            "data" => $supplier,
            "message" => "Get one supplier successfully."
        ], 200);
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, string $id)
    {
        $supplier = Supplier::find($id);

        if (!$supplier) {
            return response()->json([
                "message" => "Cetegory is not found."
            ], 404);
        }

        $validate = $request->validate([
            "name" => "required|string|min:3|max:255",
            "email" => "required|email",
            "address" => "required|string|min:2|max:255",
            "tel" => "required|string|min:3|max:255",
            "website" => "required|string|min:3|max:255",
        ]);

        $supplier->update($validate);

        return response()->json([
            "data" => $supplier,
            "message" => "Updated supplier successfully."
        ], 200);
    }

    /**
     * Remove the specified resource from storage.
     */
    public function destroy(string $id)
    {
        $supplier = Supplier::find($id);

        if (!$supplier) {
            return response()->json([
                "message" => "Cetegory is not found."
            ], 404);
        }

        $supplier->delete();

        return response()->json([
            "data" => $supplier,
            "message" => "Deleted supplier successfully."
        ], 200);
    }
}
